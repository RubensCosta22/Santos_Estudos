...
function bind(){root.querySelector('[data-close]')?.addEventListener('click',close);root.querySelector('#start')?.addEventListener('click',start);root.querySelectorAll('input[name="ans"]').forEach(i=>i.onchange=()=>{state.quiz.answers[state.quiz.ids[state.quiz.index]]=i.value;persist();render()});root.querySelector('[data-prev]')?.addEventListener('click',()=>{if(state.quiz.index>0)state.quiz.index--;render()});root.querySelector('[data-next]')?.addEventListener('click',()=>{if(state.quiz.index<69)state.quiz.index++;render()});root.querySelectorAll('[data-jump]').forEach(b=>b.onclick=()=>{state.quiz.index=+b.dataset.jump;render()});root.querySelector('#finish')?.addEventListener('click',()=>finish());root.querySelector('#done')?.addEventListener('click',()=>{state.quiz=null;persist();render()})}
function ensure(){const tabs=document.querySelector('.tp-overlay .tp-tabs');if(!tabs||tabs.querySelector('[data-real-mock]'))return;const b=document.createElement('button');b.dataset.realMock='1';b.className='tprm-tab';b.textContent='📝 Simulado';b.onclick=open;tabs.appendChild(b)}
async function init(){css();loadState();try{await normalize();window.openTranspetroRealSimulado=open;setInterval(ensure,300);ensure()}catch(e){console.error('Simulado Transpetro: erro ao carregar bancos',e)}}
init()
